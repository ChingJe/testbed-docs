...
最近更新：2026-08-27（Phase 2 placement、NF identity、eight-UE 與 reset decisions）

狀態：Phase 1 Completed (`072748b`)；Phase 2 decision baseline confirmed／implementation pending；Phase 3–4 Pending
...
- operator 必須以同一個 `CONFIG_DIR` 完成該次 scenario 的 render、validate、start、status、stop 與 reset；
...
- reset scope 必須由 selected scenario manifest／runtime inventory 產生。現有只列出 A／B／C 與五個 ML
...
| 2. Static scenario common foundation | Decision baseline confirmed；implementation pending | 在既有三台 VM 固定八個 UE、四個 data owners、獨立 NF identity 與最大 topology placement | scenario config layout、dynamic NF／ML inventories、ports、callback reachability、collection profiles、manual reset／seed restoration 與 capacity gate | 1 Server＋4 Clients 與 1 Root＋2 Branches＋4 Leaves 均可被 render／validate，所有 NF／state identity 無衝突，尚不宣稱 full flow |
