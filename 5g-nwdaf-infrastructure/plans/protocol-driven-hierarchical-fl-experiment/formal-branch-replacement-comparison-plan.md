...
狀態：Draft／待確認；正式實驗條件尚未凍結，不授權正式執行；Slice 4 第一輪已完成
...
依已決定的實驗條件逐輪擴充。共用 runner 的雙模式支援與短 epoch 簡測已完成；資料切分、評估來源與正式
訓練條件仍須完成下一輪盤點和實作，不因本計畫記錄候選值而自動獲准執行正式實驗。
...
盤點與一個可行配額候選見 Slice 4 詳細計畫：每類 4,800 筆進入 Leaves、200 筆留給 validation；
每個 Leaf 僅取五個類別、每類 1,600 筆。候選中 Area A 偏向類別 0–4，Area C 偏向 5–9，
B／C 在 Area A 故障時仍覆蓋全部十類。實作前須以實際 source indices、Leaf／Area 類別直方圖與
native loader 核對切分；此候選尚未凍結，不可看完結果才反向調整。
Non-IID 是兩種條件共同的實驗背景，不增加 IID／non-IID 兩套條件。

下一輪工具調整建議在既有 scenario 的 `partition` 內表達各 Leaf 持有的類別，沿用
...
候選在數量上可行；這不等於目前 generator 已能產生該切分。不能把增加 local epochs、重複抽樣或
資料增強描述為增加了獨立樣本。
...
validation 均為每類 200 筆；這是待 user review 的切分候選，並非已實作設定。最終模型另用
**完整的官方 10,000 筆 test split**作一次主要 held-out 評估；正式參數與 non-IID 切分凍結前
不得用這份 test 結果挑選配置。
切分建議沿用目前 scenario 的 `partition.seed: 42`：每類先固定選出 200 筆 validation，再把互斥的
4,800 筆分配給三個 Leaves；同資料集的 baseline／treatment 須得到相同的 source indices。
目前 generator 雖分別輸出 validation 與 held-out artifacts，卻把兩者都取自官方 test split、各只用
200 筆，且要求所有輸出每類別等量；正式切分必須明確調整來源與檢查規則，不能只把 scenario 的
計數改成 10,000。尤其
MNIST 官方完整 test split 各類樣本數並不相等，final evaluation 不得為了等量而丟棄 test 樣本。

## 4. 時間軸與訓練工作量

目前八輪流程只在第二個 accepted round 後故障，不足以觀察中段替換的前後曲線。第一版正式設定候選
如下；這些數值仍須在資料切分候選 review 與執行時間盤點後凍結，同資料集的 baseline／treatment 必須一致，只有
...
per-class 評估或曲線。現有 `MODEL_EVALUATION` 與 final evaluator 也只直接提供整體 loss／accuracy；
若整體指標不足以回答後續問題，再另行決定是否新增 component 記錄或離線評估，不能從整體 accuracy
推論出類別效果。
...
2. 已盤點來源類別數量並提出六份等量 label-skew shards 與固定 validation 候選；review 後凍結配額、
   seed 與正式訓練設定，再在同一 Slice 4 後續輪次完成必要的 dataset／evaluation 調整。執行前以來源索引、
   Leaf／Area 類別直方圖、互斥性、native loader 與 validation 全量覆蓋做針對性檢查，不另跑獨立
...
| 資料互斥且配對一致 | 來源索引、Leaf／Area 類別直方圖、seed、完整 validation／test count 與 native loader checks | 配對 runs 保存一致的 split 與初始模型資訊；每輪走完整固定 validation，完整官方 test 僅作凍結後的 final evaluation |
...
- 盤點後提出的五類／Leaf 配額、兩資料集各 2,000 筆 train-derived validation 候選是否凍結；
  實作後的 source indices、Leaf／Area 分布及完整 evaluation 覆蓋仍須直接驗證；
...
runner 擴充及短 epoch 簡測已完成；它們不使未決的資料與訓練條件自動獲准。
